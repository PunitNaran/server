...
This repo is the server component. If you are just looking for the CLI localtunnel app, see (https://github.com/punitnaran/localtunnel).
...
git clone git://github.com/punitnaran/server.git
cd server
npm install
...
**Note** This assumes that you have a proxy in front of the server to handle the http(s) requests and forward them to the localtunnel server on port 3000. You can use our [localtunnel-nginx](https://github.com/punitnaran/nginx) to accomplish this.
...
docker run -d \
    ...
    --name localtunnel \
    --net host \
    punitnaran/localtunnel-server:latest --port 3000
```
